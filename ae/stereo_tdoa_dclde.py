...
with graph.as_default():
    ...
    with tf.name_scope('model'):
        ...
        def model(data):
            z = enc(data)
            rolled, tdoas = permutation(z)
            return z, tdoas

    # Training computation.
    z, tdoa = model(tf_train_dataset) # + tf_train_noise)
    ...
    with tf.Session(graph=graph,config=config) as session:
        train_writer = tf.summary.FileWriter('log/stereo_tdoa_dclde/log', session.graph)
        ...
        for epoch in range(num_epoch):
            ...
            for step in trange(train_size//(batch_size)):
                ...
                _, l, lg, learn, predictions, embed, summary = session.run(
                    [optimizer, loss, loss_reg, learning_rate, z, merged], feed_dict=feed_dict)
                ...
                if (step%25 == 24):
                    tqdm.write(f'Minibatch loss at step {step}:\n total : {ml}, regularisation : {mlg}, learning rate : {mln/25}')
                    ml = mlr = mlg = mln = 0
            ...
            saver.save(session, f'log/stereo_tdoa_dclde/weight/epoch{epoch}.ckpt')