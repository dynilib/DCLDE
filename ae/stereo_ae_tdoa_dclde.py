...
with graph.as_default():
    ...
    with tf.name_scope('model'):
        ...
        def enc2(data):
            conv1 = tf.nn.conv2d(data, conv1_w2, [1,k_size1//2,1,1],'SAME')
            conv2 = tf.nn.conv3d(tf.expand_dims(conv1,4), conv2_w2, [1,1,k_size1,1,1],'SAME')
            reshape1 = tf.reshape(conv2,tf.concat([conv2.shape[:-2],[-1,1]],0))
            conv3 = tf.nn.conv3d(reshape1,conv3_w2, [1,1,2*k_size1,1,1],'SAME')
            conv4 = tf.nn.conv3d(tf.pad(conv3,[[0,0],[0,0],[k_size1**3//2 -1, k_size1**3//2],[k_size1-1, k_size1],[0,0]]), conv4_w2, [1,1,1,1,1],'VALID')
            reshape2 = tf.reshape(conv4,[batch_size,-1])
            z = tf.matmul(reshape2,d1_w2)
            return z
    ...
    with tf.Session(graph=graph,config=config) as session:
        train_writer = tf.summary.FileWriter('log/stereo_ae_tdoa_dclde/log', session.graph)
        ...
        for epoch in range(num_epoch):
            ...
            saver.save(session, f'log/stereo_ae_tdoa_dclde/weight/epoch{epoch}.ckpt')