...
def generate_batch():
    ...
    phi = 2*np.pi * np.random.random((batch_size,num_feature,1))
    batch = r.reshape(-1,1,1) * np.concatenate((np.cos(phi),np.sin(phi)),2)
    return batch
...
with graph.as_default():
    ...
    with tf.name_scope('Variables'):
        conv1_w = tf.Variable(tf.truncated_normal(
            [1,2,1,16], stddev=0.1), name='conv1_weight')
        conv2_w = tf.Variable(tf.truncated_normal(
            [1,2,16,1], stddev=0.1), name='conv2_weight')
    ...
    with tf.name_scope('model'):
        def enc(input):
            conv1 = tf.nn.leaky_relu(tf.nn.conv2d(tf.expand_dims(input,-1), conv1_w, [1,1,1,1],'SAME'))
    ...
    with tf.name_scope('loss'):
        train_complex = tf.complex(tf_train_dataset[:,:,0],tf_train_dataset[:,:,1])
        pred_complex = tf.complex(prediction[:,:,0],prediction[:,:,1])
        loss_mean = tf.losses.mean_squared_error(tf.reduce_mean(tf.abs(train_complex),-1), tf.reduce_mean(tf.abs(pred_complex),-1))
        loss_std = tf.reduce_mean(tf.keras.backend.std(tf.abs(pred_complex),-1))
    ...
    with tf.Session(graph=graph,config=config) as session:
        #train_writer = tf.summary.FileWriter('ae_circle_dist', session.graph)
        ...
        for step in trange(num_step):
            ...
            _, l, m, s = session.run([optimizer, loss, loss_mean, loss_std], feed_dict=feed_dict)
            ml += l
            mm += m
            ms += s
            if (step%num_visual_step == num_visual_step -1):
                ...
                mm/=num_visual_step
                ms/=num_visual_step
                tqdm.write(f'Minibatch mean loss from step {step +1 -num_visual_step} to step {step}: total : {ml}, loss_mean : {mm}, loss_std : {ms}')
                ml=mm=ms=0
